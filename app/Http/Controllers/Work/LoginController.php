...
class LoginController extends Controller
{
    ...
    public function register(Request $request)
    {
        ...
        //验证email
        $u = Login::where(['email'=>$email])->first();
        if($u){
            $response = [
                'errno' => 500003,
                'msg'   => "Email已被使用"
        ...
        //验证mobile
        $u = Login::where(['mobile'=>$mobile])->first();
        if($u){
            $response = [
                'errno' => 500003,
                'msg'   => "电话号已被使用"
        ...
        $uid = Login::insertGetId($user_info);
        if($uid)
        {
            $response = [
                'errno' => 0,
    ...
    public function login(Request $request)
    {
        ...
        // 按name找记录
        $u1 = Login::where(['name'=>$value])->first();
        $u2 = Login::where(['email'=>$value])->first();
        $u3 = Login::where(['mobile'=>$value])->first();
